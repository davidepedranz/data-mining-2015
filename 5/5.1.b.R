...
d <- function(i,j) {
  ...
  return( w1 * abs(p1[1] - p2[1]) + w2 * abs(p1[2] - p2[2]) )
}
...
q <- function(c1, c2) {
  ...
  for(i in 1:dim(pp)[1]) {
    ...
    print(paste("x", i, " --> cluster ", ifelse(d1 < d2, c1, c2),
                " -- dist=", formatC(min(d1,d2),digits=2, format="f"),
                " [ ", formatC(d1,digits=2, format="f") , ", ",
                formatC(d2,digits=2, format="f"), " ]", sep = ""));
  }
  ...
  print("")
  print(paste("total distance =", formatC(tot,digits=2, format="f")))
}